...
app.get('/SearchIdc/:member_Id', (req, res) => {
  ...
  IdcDAO.SearchIdc(member_id, (error, idc) => {
    if (error) {
      res.status(500).json({error: '학생증 조회 중 오류가 발생했습니다.'});
...
const handleSign = (req, res) => {
  ...
  const {id, pw, name, nickname, engname, email, major, birth, gender, grade} =
    req.body;
...
//-------------------
//--------게시글 관련-----------
app.post('/CreateBoard', BoardDAO.CreateBoard); // 게시글 생성을 클라이언트로부터 요청받아 실행
app.get('/BoardList', (req, res) => {
  //게시글 목록을 클라이언트로 부터 요청받아 조회한 데이터를 반환
...
app.get('/BoardList/:category', (req, res) => {
  // 게시글 목록(카테고리)을 클라이언트로 부터 요청받아 조회한 데이터를 반환
...
app.post('/BoardSearchList', (req, res) => {
  //게시글 검색을 클라이언트로 부터 요청받아 조회한 데이터를 반환
...
app.get('/BoardHitsUpdate/:boardId', (req, res) => {
  // 조회수 증가을 클라이언트로 부터 요청받아 DB에 Update함
  const boardId = req.params.boardId;
...
app.get('/BoardDetail/:boardId', (req, res) => {
  //게시글 상세보기을 클라이언트로 부터 요청받아 조회한 데이터를 반환
  const boardId = req.params.boardId;
...
app.get('/BoardDetailUpdate/:boardId', (req, res) => {
  //게시글 상세보기(좋아요 변경시)을 클라이언트로 부터 요청받아 조회한 데이터를 반환
  const boardId = req.params.boardId;
  // 게시글 상세보기
  BoardDAO.BoardDetail(boardId, (error, board) => {
    if (error) {
...
app.post('/LikePlus', (req, res) => {
  //좋아요 추가을 클라이언트로 부터 요청받아 DB에 Create함
...
//게시글 수정을 클라이언트로 부터 요청받아 DB에 Update함

//-----------------------------------------------------------------------------------------
// ... 댓글 코드 ...
app.post('/CommentAdd', CommentDAO.CreateComment);
// 댓글 생성을 클라이언트로 부터 요청받아 DB에 Create함

app.get('/CommentList/:boardId', (req, res) => {
  // 댓글 조회 (특정 게시글의 모든 댓글을 가져오는 가정)을 클라이언트로 부터 요청받아 조회한 데이터를 반환
  const boardId = req.params.boardId;
...
//댓글 수정을 클라이언트로 부터 요청받아 DB에 Update함
app.delete('/DeleteComment/:comment_id', (req, res) => {
  //댓글 삭제을 클라이언트로 부터 요청받아 DB에서 제거함
...
app.get('/ReplyList/:boardId', (req, res) => {
  // 답글 조회 (특정 게시글의 모든 댓글을 가져오는 과정)을 클라이언트로 부터 요청받아 조회한 데이터를 반환
  const boardId = req.params.boardId;
...
//답글 생성을 클라이언트로 부터 요청받아 DB에 Create함
app.post('/EditReply/', ReplyDAO.EditReply);
//답글 수정을 클라이언트로 부터 요청받아 DB에 Update함
app.delete('/DeleteReply/:reply_id', (req, res) => {
  //답글 삭제을 클라이언트로 부터 요청받아 DB에서 제거함
