...
const CreateBoardLikes = (req, res) => {
  //DB에 입력받은 회원 id와 게시글 id를 가지는 테이블을 생성한다.
...
const SelectUserBoardLike = (req, res, callback) => {
  //DB에 게시글 id와 회원 id를 같이 가지고 있는 테이블이 있는지 확인한다.
  const memberId = req.body.memberId;
  const boardId = req.body.boardId;
...
const DeleteBoardLike = (req, res, callback) => {
  //DB에서 게시글 id와 회원 id를 같이 가지고 있는 테이블을 제거한다.
  const memberId = req.body.memberId;
  const boardId = req.body.boardId;
...
const CountBoardLikes = (boardId, callback) => {
  //DB에서 게시글 id를 가지고 있는 테이블의 개수를 조회한다.
  const query =
