...
const CreateBoard = ({navigation}) => {
  ...
  const [boardid, setBoardId] = useState(''); //게시글 id
  const [content, setContent] = useState(''); //게시글 내용
  const [mid, setMid] = useState(''); //게시글 작성자
  const [category, setCategory] = useState(''); // 게시글 카테고리
  const [id, setId] = useState(''); //회원 id
  const [name, setName] = useState(''); //회원 이름
  const [isAdmin, setIsAdmin] = useState(null); //관리자 여부
  const [major, setMajor] = useState(''); //회원의 학과
  const [categorySub, setCategorySub] = useState(''); //게시글 subCategru
  const [response, setResponse] = useState(null); //서버에서 불러온 데이터
  const [showOverlay, setShowOverlay] = useState(false);
  const [selectCategoryOption, setSelectCategoryOption] =
    useState('카테고리 선택'); //대분류 카테고리 선택
  const [selectCategorySubOption, setSelectCategorySubOption] =
<<<<<<< HEAD
    useState('카테고리 선택'); //소분류 카테고리 선택
=======
    useState('카테고리 선택');
  //카테고리와 서브카테고리를 저장하는 배열들을 선언하였습니다.
>>>>>>> 37a1ee46b6fd4ce5b5f4e586cccb16fed86717b1
  ...
  const categoryAdmin = ['학사일정'];
  const categorySubNotice = ['공지'];
...
>>>>>>> 37a1ee46b6fd4ce5b5f4e586cccb16fed86717b1
  ...
  const createNewBoard = async () => {
    //이미지가 존재하지 않을 경우 DB에 게시글을 추가하는 함수
    try {
  ...
  useEffect(() => {
<<<<<<< HEAD
    //선택 학과 선택시 로그인한 유전의 학과 정보를 불러와서 카테고리에 추가함
=======
