...
const BoardDetail = ({route, navigation}) => {
  const apiUrl =
    'https://port-0-polintechservercode-ac2nlkzlq8aw.sel4.cloudtype.app';
  ...
  const [trigger, setTrigger] = useState(false); //변경을 확인하는 변수 - 이 변수를 통해서 게시글 상세보기 함수를 재실행한다.
  ...
  const handleLike = async () => {
    //좋아요 추가 함수 이때 해당 게시글에 이미 좋아요를 누른 상태일 경우 좋아요를 취소한다.
    ...
    await fetch(`${apiUrl}/LikePlus`, {
      ...
      .then(data => {
        if (data.success) {
          ...
          setTrigger(prev => !prev); //변경을 확인하는 변수를 수정함 -
        }
      })
      .catch(error => {
            ...
            <View style={styles.block}>
              <View style={styles.header}>
                <Text style={styles.txtTitle}>
                  [{board.board_subcategory}]{board.board_title}
                </Text>
                <View
                  style={{
                    flexDirection: 'row',
                ...
                <Text>
                  조회 {board.board_hits} | 좋아요 {likeCount}
                </Text>
              </View>
