...
function MainTest({navigation, route}) {
      ...
      if (category === '전체') {
        ...
        try {
          const response = await fetch(
            'https://port-0-polintechservercode-ac2nlkzlq8aw.sel4.cloudtype.app/BoardList',
          );
        ...
        try {
          const response = await fetch(
            `https://port-0-polintechservercode-ac2nlkzlq8aw.sel4.cloudtype.app/BoardList/${category}`,
          );
          const json = await response.json();
          if (json.success) {
            setBoards(json.boards);
  ...
  useEffect(() => {
    const fetchNotices = async () => {
      try {
        const response = await fetch(
          'https://port-0-polintechservercode-ac2nlkzlq8aw.sel4.cloudtype.app/BoardList/학사일정',
        );
        const json = await response.json();
        if (json.success) {
