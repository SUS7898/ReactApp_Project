...
const CommentEdit = ({route, navigation}) => {
  ...
  const comment_mid = route.params.board_title; //댓글 작성자 id
  const comment_content = route.params.comment_content; //댓글 내용
  ...
  const updateComment = async () => {
    //댓글 수정 - DB에서 해당하는 댓글 id를 가진 댓글을 수정함
    try {
      const response = await fetch(
        `https://port-0-polintechservercode-ac2nlkzlq8aw.sel4.cloudtype.app/EditComment`,
        {
          method: 'POST',
          headers: {
  ...
  const DeleteComment = async () => {
    //댓글 삭제 - DB에서 해당하는 댓글 id를 가진 댓글을 삭제함
    try {
