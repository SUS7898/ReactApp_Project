...
const BoardEdit = ({route, navigation}) => {
  const boardId = route.params.board_id; //해당 게시글 id
  const oTitle = route.params.board_title; //원본 게시글 제목
  const oContent = route.params.board_content; //원본 게시글 내용
  const image_name = route.params.image_name; //이미지 파일 이름
  const isImage = route.params.isImage; //기존에 이미지 여부
  const member_id = route.params.member_id; //로그인한 회원 id
  const category = route.params.category; //현재 게시글의 카테고리
  // const otrigger = route.params;
  const {setTrigger} = route.params; //변경 확인 변수
  ...
  const updateBoard = async () => {
    //게시글 수정 - 게시글 id를 통해서 해당하는 게시글 제목, 게시글 내용을 수정한다.
    try {
      const response = await fetch(
        `https://port-0-polintechservercode-ac2nlkzlq8aw.sel4.cloudtype.app/EditBoard`,
        {
          method: 'POST',
          headers: {
  ...
  const handleBoardImage = async boardId => {
    ...
    */
    try {
      ...
      // isImage에 따라 이미지가 존재하면 이미지 수정 없으면 이미지 추가를 실행
      const endPoint = isImage
        ? 'UpdateBoardImageToDB'
        : 'UploadBoardImageToDB';
      const toBoardDB = await fetch(
        `https://port-0-polintechservercode-ac2nlkzlq8aw.sel4.cloudtype.app/${endPoint}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
